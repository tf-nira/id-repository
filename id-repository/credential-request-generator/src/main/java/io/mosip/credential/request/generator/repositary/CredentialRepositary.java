...
//	@Lock(value = LockModeType.PESSIMISTIC_WRITE) 
	@QueryHints({ @QueryHint(name = "javax.persistence.lock.timeout", value = "1") })
//	@Query("select c from CredentialEntity c where c.statusCode=:statusCode")
	@Query(value = "SELECT * FROM credential_transaction c WHERE c.status_code = :statusCode and c.issuer  not in :issuers ORDER BY c.cr_dtimes FOR UPDATE SKIP LOCKED LIMIT :batchSize", nativeQuery = true)
